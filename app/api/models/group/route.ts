...
import { generateGroupOutput, type ModelSettings } from "@/lib/llm";
...
export async function POST(req: Request) {
  try {
    ...
    if (!input.trim() || !group.trim()) {
      return NextResponse.json(
        { error: "缺少参数：input 或 group 不能为空。" },
        { status: 400 },
      );
    }

    const content = await generateGroupOutput(
      input,
      group,
      context,
      modelType,
